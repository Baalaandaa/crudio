...
export default class MongoDB implements Database {
  ...
  constructor(uri: string, dbName: string, connectionOptions?: MongoClientOptions) {
    this.dbName = dbName;
  ...
  delete(schema: string, filterOptions?: any): Promise<boolean> {
    return new Promise<boolean>((resolve, reject) => {
      this.client.db(this.dbName).collection(schema).findOneAndDelete({
        ...filterOptions
      }, (err) => {
        if(err)
          reject(err);
  ...
  getAll(schema: string, filterOptions?: any): Promise<any[]> {
    return new Promise<any[]>(async (resolve) => {
      let result = await this.client.db(this.dbName).collection(schema).find({
        ...filterOptions
      });
  ...
  insert(schema: string, data: Object): Promise<any> {
    return new Promise<any>(async (resolve, reject) => {
      try {
        let result = await this.client.db(this.dbName).collection(schema).insertOne(data);
